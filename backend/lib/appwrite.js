...
try {
  ...
  const documentData = {
    ...
    status: 'pending' // Add the required status field with default value 'pending'
  };
...
// Improved updateDefectStatus function with better debugging
export async function updateDefectStatus(documentId, status) {
try {
  console.log(`Updating defect status - Document ID: "${documentId}", Status: "${status}"`);
  ...
  console.error(`Error updating defect status for ID "${documentId}":`, error);
  throw new Error(`updateDefectStatus error: ${error.message}`);
}
}